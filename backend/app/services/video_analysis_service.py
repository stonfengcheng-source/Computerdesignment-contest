...
def process_video_for_inconsistency(video_path: str, player_id: str):
    ...
    if extracted_texts:
        for text in extracted_texts:
            clean_text = text.split("：")[-1].lower() if "：" in text else text.lower()

            # 🌟 核心修复 1&2：移除了本地的 BLACK_LIST，直接调用底层引擎
            # 这样视频流和图谱共享同一套随机波动 (0.85~0.95) 与判毒标准！
    ...
    # 🌟 核心修复 3：行为流彻底砍掉，最终毒性完全等于最高文本毒性！
    text_prob = max_text_prob
    behavior_error = 0.0  # 保留变量名为了防止其他接口报错，但直接给0
    toxicity_score = text_prob
